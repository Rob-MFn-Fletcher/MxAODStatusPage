...
Samples=()
for DIR in ${MXAODDIRS[@]}; do
  Samples+=($(eos ls $datasetDir/$htagNew/$DIR/ | grep "Sherpa_gamjet_35-70" ))
  #break
done
...
for filename in ${Samples[@]}; do
  ...
  [[ -z "$oldCutflowName" ]] && echo $htagOld version of $fileType does not exist! skipping...  && continue
  #newCutflowName=$(echo $filename | sed "s/h[0-9][0-9][0-9]/$htagNew/g")
  ./diffCutflow.py ${oldCutflowName}.txt ${filename}.txt && \
